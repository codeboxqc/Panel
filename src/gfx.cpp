...
void putimage(int px, int py, const unsigned char* img) {
    for(int y = 0; y < 64; y++) {
        ...
        for(int x = 0; x < 64; x++) {
            ...
            if(col != 0) {
                setBufferPixel(screenX, screenY, palette[col]); // <--
            }
        }
    }
}
...
// =========================
// Modified putimagesize (with RGB averaging)
// =========================
...
void putimageColorMap(int px, int py, const unsigned char* img, int color) {
    for(int y = 0; y < 64; y++) {
        ...
        for(int x = 0; x < 64; x++) {
            ...
            uint8_t col = getPixel(img, idx);
            if(col != 0) {
                setBufferPixel(screenX, screenY, palette[color]); // <--
            }
        }
    }
}
...
void putimageFade(int px, int py, const unsigned char* img, float fade) {
    ...
    for(int y = 0; y < 64; y++) {
        ...
        for(int x = 0; x < 64; x++) {
            ...
            uint8_t col = getPixel(img, idx);
            if(col != 0) {
                uint16_t rgb = palette[col];
                uint16_t r = ((rgb >> 11) & 0x1F) * fade;
                uint16_t g = ((rgb >> 5) & 0x3F) * fade;
                uint16_t b = (rgb & 0x1F) * fade;
